...
protocol StorageService {
    ...
    func savePermissions(_ permission: ApplicationState.PermissionData)
    func markLaunched()
...
final class UserDefaultsStorageService: StorageService {
    private let store = UserDefaults(suiteName: "group.homezone.storage")!
    private let cache = UserDefaults.standard
    
    private enum Key {
        static let tracking = "hz_tracking_payload"
        static let navigation = "hz_navigation_payload"
        static let endpoint = "hz_endpoint_target"
        static let mode = "hz_mode_active"
        static let firstLaunch = "hz_first_launch_flag"
        static let permGranted = "hz_perm_granted"
        static let permDenied = "hz_perm_denied"
        static let permDate = "hz_perm_date"
    }
...
final class SupabaseValidationService: ValidationService {
    ...
    init() {
        self.client = SupabaseClient(
            supabaseURL: URL(string: "https://edbjalzmqoyqlphvsbza.supabase.co")!,
            supabaseKey: "sb_publishable_SOhT6TtdMyr_i0jwGB4bWA_Qie3nTuA"
        )
    }
...
final class HTTPNetworkService: NetworkService {
    ...
    func fetchAttribution(deviceID: String) async throws -> [String: Any] {
        var builder = URLComponents(string: "https://gcdsdk.appsflyer.com/install_data/v4.0/id\(HomeZoneConfig.appID)")
        builder?.queryItems = [
            URLQueryItem(name: "devkey", value: HomeZoneConfig.devKey),
    ...
    func fetchEndpoint(tracking: [String: Any]) async throws -> String {
        guard let url = URL(string: "https://homezonne.com/config.php") else {
            throw NetworkError.invalidURL
        }
        ...
        payload["store_id"] = "id\(HomeZoneConfig.appID)"
        ...
        var lastError: Error?
        let retries: [Double] = [30.0, 60.0, 120.0]
...
struct HomeZoneConfig {
    static let appID = "6762137922"
    static let devKey = "m6Lp4m9zyZ5QPHJ5JgbVKk"
}
